...
(router = express()), (bp = require("body-parser"));
router.use(express.static("views"));
router.use(bp.urlencoded({
        extended: true
    })),
    (trainedData = require("./prefixTree"));
        ...
        let data = await new Promise(resolve => {
            require("fs").readFile("words.txt", "utf-8", (err, data) => {
                ...
                for (let i = 0; i < woN.length; i++) {
                    ...
                    if (word.length === 1 && woN[i].trim().length !== 0)
                        arr.push(woN[i].substring(0, woN[i].length - 1).toLowerCase());
                }
                resolve(arr);
            });
        });
